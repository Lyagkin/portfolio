...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const showArrow = function (entries) {
    // показываем стрелку, когда секция resume пересекает границу наблюдателя
    const [entry] = entries;
  ...
  const arrowObserver = new IntersectionObserver(showArrow, {
    // задаем настройки границы срабатывания наблюдателя
    root: null,
    threshold: 0,
  });
  ...
  const hideArrow = function (entries) {
    // скрываем стрелку, когда секция promo пересекает границу наблюдателя
    const [entry] = entries;
  ...
  const sectionAboutObserver = new IntersectionObserver(hideArrow, {
    // задаем настройки границы срабатывания наблюдателя
    root: null,
    threshold: 0,
  });
  ...
  arrowLink.addEventListener("click", (e) => {
    e.preventDefault(); // отменяем переход по ссылке к главной странцие

    const arrowId = arrowLink.getAttribute("href"); // получаем значение атрибута ссылки

    document.querySelector("" + arrowId).scrollIntoView({
      // скроллим к элементу со значением из атрибута ссылки
      behavior: "smooth",
      block: "start",
    });
  });
